...
class FiringAdmin extends StatefulWidget {
  const FiringAdmin({super.key});
  @override
  State<FiringAdmin> createState() => _FiringAdminState();
}

class _FiringAdminState extends State<FiringAdmin> {
  final _db = AppDatabase.instance;
  String? _armyNo, _soldierName, _soldierRank, _soldierCoy;
  bool _saving = false;
  String? _result;
  List<FiringRecord> _history = [];
  ...
  Future<void> _onSoldierSelected(
    ...
    _history = await _db.getFiringByArmyNo(an);
    setState(() {});
  }
  ...
  Future<void> _submit() async {
    ...
    final rec = FiringRecord(armyNo: _armyNo!)..result = _result;
    await _db.insertFiring(rec);
    _history = await _db.getFiringByArmyNo(_armyNo!);
    setState(() {
      _result = null;
    });
    if (mounted) {
      showSnack(context, 'Firing result added.');
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Container(
          ...
          child: const Text('FIRING RESULTS', style: kSectionTitle)),
      Expanded(
            ...
            child: SingleChildScrollView(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      if (_armyNo != null) ...[
                        ...
                        Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                border:
                                    Border.all(color: kGold.withOpacity(.35))),
                            child: Text(
                      ...
                      const SectionHeader('Add Firing Result'),
                      const SizedBox(height: 12),
                      _dd('Result', _result, kFiringResults,
                            ...
                            trailing: IconButton(
                                ...
                                onPressed: () async {
                                  final ok = await confirmDialog(context,
                                      'Delete', 'Remove this firing result?');
                                  if (!ok) return;
                                  await _db.deleteFiring(r.id!);
                                  _history =
                                      await _db.getFiringByArmyNo(_armyNo!);
                                  setState(() {});
                                }));
                      })),
        ])),
