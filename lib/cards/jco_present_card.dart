...
class _JcoPresentCardState extends State<JcoPresentCard>
  ...
  String? _rank, _coy, _bloodGp, _svcExtn, _photoPath, _domicile;
  ...
  // Army Edn
  String? _eMr1, _eMr2, _eMr3, _eAce1, _eAce2, _eAce3, _eAec3;
  // Promo Cadres
  ...
  void _populate(JcoOrModel r) {
    ...
    _aadhar.text = r.aadharCard ?? '';
    _diag.text = r.diag ?? '';
    _dueOn.text = r.dueOn ?? '';
    _email.text = r.emailId ?? '';
    _father.text = r.father ?? '';
    _mother.text = r.mother ?? '';
    _wife.text = r.wife ?? '';
    _nok.text = r.nextOfKin ?? '';
    _acct.text = r.acctNo ?? '';
    _bank.text = r.bankName ?? '';
    ...
    _pAcp.text = r.pAcp ?? '';
    final en = [r.ere1Name, r.ere2Name, r.ere3Name];
    final ef = [r.ere1From, r.ere2From, r.ere3From];
    final et = [r.ere1To, r.ere2To, r.ere3To];
    for (int i = 0; i < 3; i++) {
      _ereN[i].text = en[i] ?? '';
      _ereF[i].text = ef[i] ?? '';
      _ereT[i].text = et[i] ?? '';
    ...
    final cn = [r.ch1Name, r.ch2Name, r.ch3Name, r.ch4Name];
    final cd = [r.ch1Dob, r.ch2Dob, r.ch3Dob, r.ch4Dob];
    for (int i = 0; i < 4; i++) {
      _chN[i].text = cn[i] ?? '';
      _chD[i].text = cd[i] ?? '';
    }
  ...
  JcoOrModel _buildModel() {
    ...
    m.aadharCard = _aadhar.text.trim();
    m.diag = _diag.text.trim();
    m.dueOn = _dueOn.text;
    m.emailId = _email.text.trim();
    ...
    m.acctNo = _acct.text.trim();
    m.bankName = _bank.text.trim();
  ...
  Widget _fMulti(String l, TextEditingController c, {double w = 440}) =>
      SizedBox(
          width: w,
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  // ── Flex variants (no fixed width) — used for PERS DETAILS so every row
  // fills its full width edge-to-edge with no leftover gaps.
  ...
  Widget _ffMulti(String l, TextEditingController c, {int lines = 2}) => Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
          children: [
            Padding(
  ...
  Widget _childTable() => Table(columnWidths: const {
        ...
        TableRow(
            children: [_tHdr('Children Name'), _tHdr('Gender'), _tHdr('DOB')]),
        for (int i = 0; i < 4; i++)
          TableRow(children: [
            Padding(
  ...
  Widget _discTable() => Table(columnWidths: const {
        0: FlexColumnWidth(4),
        1: FlexColumnWidth(3),
        2: FlexColumnWidth(2),
        3: FlexColumnWidth(1)
    ...
    return Column(children: [
            ...
            Expanded(
                child: Column(
                  ...
                  _fRow([
                    ...
                    (
                      1,
                      _ffDD('Svc Extn', _svcExtn, kYesNo,
                          (v) => setState(() => _svcExtn = v))
                    ),
                  ...
                  _fRow([
                    ...
                    (2, _ff('Aadhar Card', _aadhar)),
                    (2, _ff('Diag', _diag)),
                    (1, _ffDate('Due On', _dueOn))
                  ]),
                  const SizedBox(height: 14),
                  _fRow([
                    (1, _ff('E Mail Id', _email)),
      ...
      CardSection(
          ...
          child: Wrap(spacing: 14, runSpacing: 14, children: [
            _f('Acct No', _acct, w: 240),
            _f('Bank Name', _bank, w: 310),
          ])),
      // Home Address
      CardSection(
          title: 'Home Address',
