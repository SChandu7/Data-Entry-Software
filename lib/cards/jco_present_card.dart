...
class JcoPresentCard extends StatefulWidget {
  ...
  const JcoPresentCard({super.key, this.record, required this.onSaved});
  @override
...
class _JcoPresentCardState extends State<JcoPresentCard> {
  ...
  final _medCat=TextEditingController(); final _aadhar=TextEditingController();
  ...
  final _furLve=TextEditingController(); final _notAv=TextEditingController();
  // Bank
  ...
  void dispose() {
    for (final c in [_armyNo,_name,_dob,_doe,_dor,_tos,_rrEre,_icard,_honours,_pan,
        _caste,_civEdn,_medCat,_aadhar,_diag,_dueOn,_email,_father,_mother,_wife,_nok,
        _avCl,_avAl,_furLve,_notAv,_acct,_bank,
  ...
  void _populate(JcoOrModel r) {
    ...
    _tos.text=r.tos??''; _rrEre.text=r.rrEreFmn??'';
    _icard.text=r.icardNo??''; _honours.text=r.honoursAwards??'';
    _pan.text=r.panCardNo??''; _caste.text=r.caste??'';
    _civEdn.text=r.civEdn??''; _medCat.text=r.medCat??'';
    _aadhar.text=r.aadharCard??''; _diag.text=r.diag??'';
    _dueOn.text=r.dueOn??''; _email.text=r.emailId??'';
    _father.text=r.father??''; _mother.text=r.mother??'';
    _wife.text=r.wife??''; _nok.text=r.nextOfKin??'';
    _avCl.text=r.availedCl??''; _avAl.text=r.availedAl??'';
    _furLve.text=r.furLve??''; _notAv.text=r.notAvailedLve??'';
    _acct.text=r.acctNo??''; _bank.text=r.bankName??'';
    _hTele.text=r.homeTele??''; _hVill.text=r.homeVillage??'';
    _hPost.text=r.homePost??''; _hToff.text=r.homeTOff??'';
    _hTeh.text=r.homeTehsil??''; _hDist.text=r.homeDistrict??'';
    ...
    _pNbSub.text=r.pNbSub??''; _pSub.text=r.pSub??'';
    _pSubMaj.text=r.pSubMaj??''; _pAcp.text=r.pAcp??'';
    final en=[r.ere1Name,r.ere2Name,r.ere3Name];
    final ef=[r.ere1From,r.ere2From,r.ere3From];
    final et=[r.ere1To,r.ere2To,r.ere3To];
    for(int i=0;i<3;i++){_ereN[i].text=en[i]??'';_ereF[i].text=ef[i]??'';_ereT[i].text=et[i]??'';}
    ...
    for(int i=0;i<4;i++){_chN[i].text=cn[i]??'';_chD[i].text=cd[i]??'';}
    setState(() {
      _rank=r.rank; _coy=r.coy; _bloodGp=r.bloodGp; _svcExtn=r.serviceExtn;
  ...
  void _clear() {
    ...
    setState(() {
      _rank=null;_coy=null;_bloodGp=null;_svcExtn=null;_photoPath=null;
  ...
  JcoOrModel _buildModel() {
    final m = JcoOrModel(subCategory: SubCat.jcoPresent, id: widget.record?.id);
    ...
    m.aadharCard=_aadhar.text.trim(); m.diag=_diag.text.trim();
    m.dueOn=_dueOn.text; m.emailId=_email.text.trim(); m.photoPath=_photoPath;
    ...
    m.furLve=_furLve.text; m.notAvailedLve=_notAv.text;
    ...
    m.cJlc=_cJlc.text;m.cPc=_cPc.text;m.cPt=_cPt.text;
    m.cTpt=_cTpt.text;m.cMisc=_cMisc.text;
  ...
  Widget _f(String l, TextEditingController c, {double w=210}) => SizedBox(width:w,
    ...
    children: [Padding(padding:const EdgeInsets.only(bottom:4),
  ...
  Widget _fMulti(String l, TextEditingController c, {double w=440}) => SizedBox(width:w,
    child: Column(crossAxisAlignment:CrossAxisAlignment.start, mainAxisSize:MainAxisSize.min,
    children: [Padding(padding:const EdgeInsets.only(bottom:4),
        child:Text(l.toUpperCase(),style:kLabelStyle)),
  ...
  Widget _dd(String l, String? val, List<String> opts, void Function(String?) cb, {double w=170}) =>
    SizedBox(width:w, child: Column(crossAxisAlignment:CrossAxisAlignment.start, mainAxisSize:MainAxisSize.min,
    children: [Padding(padding:const EdgeInsets.only(bottom:4),
        child:Text(l.toUpperCase(),style:kLabelStyle)),
  ...
  Widget _childTable() => Table(
    ...
    children:[
      TableRow(children:[_tHdr('Children Name'),_tHdr('Sex'),_tHdr('DOB')]),
      for(int i=0;i<4;i++) TableRow(children:[
        ...
        Padding(padding:const EdgeInsets.only(bottom:8),
            child:TextField(controller:_chD[i],readOnly:true,style:kFieldStyle,
                onTap:()=>_pd(_chD[i]),
                decoration:kDec().copyWith(suffixIcon:const Icon(Icons.calendar_today_outlined,size:14)))),
      ]),
    ]);
  ...
  Widget build(BuildContext context) {
    return Column(children:[
      Container(width:double.infinity, padding:const EdgeInsets.symmetric(vertical:10,horizontal:14),
        ...
        child:const Text('PRESENT JCOs/OR : DATA CARD',
      ...
      CardSection(title:'Pers Details', child:Column(children:[
        Row(crossAxisAlignment:CrossAxisAlignment.start, children:[
          Expanded(child:Wrap(spacing:14,runSpacing:14,children:[
            ...
            _f('Name',_name,w:290),
            _fDate('DOB',_dob,w:150),_fDate('DOE',_doe,w:150),_fDate('DOR',_dor,w:150),_fDate('TOS',_tos,w:150),
            ...
            _f('Aadhar Card',_aadhar,w:210), _f('Diag',_diag,w:200),
            _fDate('Due On',_dueOn,w:150), _f('E Mail Id',_email,w:270),
      ...
      CardSection(title:'Leave Details', child:Wrap(spacing:14,runSpacing:14,children:[
        ...
        _f('Fur Lve (Days)',_furLve,w:190),_f('Not Availed Lve Last (Days)',_notAv,w:240),
      ])),
      // Bank Details
      ...
      BipFooter(
        isEditing:widget.record!=null, isSaving:_saving,
